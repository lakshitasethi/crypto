...
const App = () => {
  const [increase, setIncrease] = useState(0.0);
  const [progressBarSize, setProgressBarSize] = useState(0.0);

  useEffect(() => {
    const loadValues = async () => {
      try {
        const value = await AsyncStorage.getItem("increase");
        const progressBarValue = await AsyncStorage.getItem("progressBar");

        if (value !== null) {
          setIncrease(parseFloat(value));
        }

        if (progressBarValue !== null) {
          setProgressBarSize(parseFloat(progressBarValue));
        }
      } catch (e) {
  ...
  const handleIncrement = async () => {
    Vibration.vibrate();
    try {
      const newIncrease = parseFloat((increase + 1).toFixed(8));
      const newProgressBarSize = parseFloat(
        (progressBarSize + 0.25).toFixed(4)
      );

      if (newIncrease <= 4) {
        setIncrease(newIncrease);
        setProgressBarSize(newProgressBarSize);

        await AsyncStorage.setItem("increase", newIncrease.toString());
        await AsyncStorage.setItem(
          "progressBar",
          newProgressBarSize.toString()
        );
      } else {
        console.log("Increase value cannot exceed 4");
      }
    } catch (e) {
      console.error("Failed to save values to AsyncStorage", e);
    }
  };
    ...
    const shareOnTelegram = () => {
      ...
      const telegramURL = `https://telegram.me/${telegramUsername}?text=${encodeURIComponent(message)}`;
      
      // Open the Telegram app with the constructed URL
      Linking.openURL(telegramURL).catch(err => console.error('An error occurred', err));
    };
        ...
        <SafeAreaView style={{ flex: 1, paddingHorizontal: 10 }}>
          <View style={styles.mainView}>
            ...
            >
              ...
              <View
                style={[
                  styles.energyBackgroundArea,
                  { marginTop: 60, paddingRight: 18 },
                ]}
              >
                <View
                  style={[
                ...
                >
                  ...
                  >
                    <Image
                      ...
                      source={require("./src/assets/images/Users.png")}
                    ></Image>
                  </View>
                  <View
                    style={{
                  ...
                  >
                    <View>
                      ...
                      <ProgressBar
                        progress={0.976}
                        color="#D6A13A"
                        style={styles.progressBarSections}
                      />
                    </View>
                    <View>
                      {/* <BrightnessComp /> */}
                    ...
                    <View>
                      ...
                      <ProgressBar
                        progress={0.0}
                        style={[
                          styles.progressBarSections,
                          { backgroundColor: "#4B3218" },
                        ]}
                      />
                    </View>
                    <View>
